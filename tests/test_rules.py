...
class TestFairHousingRule:
    ...
    @pytest.mark.asyncio
    async def test_fair_housing_success(self, mock_openai_client, mock_openai_response, mock_prompt_data):
    ...
    @pytest.mark.asyncio
    async def test_fair_housing_with_violation(self, mock_openai_client, mock_prompt_data):
...
```"""
    ...
    @pytest.mark.asyncio
    async def test_fair_housing_formats_prompt_correctly(self, mock_openai_client, mock_openai_response, mock_prompt_data):
        """Test that Fair Housing rule formats the prompt and calls OpenAI correctly."""
        mock_openai_client.responses.create = AsyncMock(return_value=mock_openai_response)
        with patch('app.rules.base.client', mock_openai_client):
            ...
            # The current code uses 'input' with a rendered Jinja template
            input_messages = call_args.kwargs['input']
            assert isinstance(input_messages, list)
            assert len(input_messages) == 1
            assert input_messages[0]['role'] == 'system'
            # The rendered template should contain the remark values
            content = input_messages[0]['content']
            assert "Beautiful home" in content
            assert "Must see" in content

    @pytest.mark.asyncio
    async def test_fair_housing_retries_on_failure(self, mock_openai_client, mock_openai_response, mock_prompt_data):
...
class TestCompRule:
    ...
    @pytest.mark.asyncio
    async def test_comp_rule_success(self, mock_openai_client, mock_openai_response, mock_prompt_data):
    ...
    @pytest.mark.asyncio
    async def test_comp_rule_with_violation(self, mock_openai_client, mock_prompt_data):
...
```"""
    ...
    @pytest.mark.asyncio
    async def test_comp_rule_formats_prompt_correctly(self, mock_openai_client, mock_openai_response, mock_prompt_data):
        """Test that COMP rule formats the prompt and calls OpenAI correctly."""
        mock_openai_client.responses.create = AsyncMock(return_value=mock_openai_response)
        with patch('app.rules.base.client', mock_openai_client):
...
class TestMarketingRule:
    ...
    @pytest.mark.asyncio
    async def test_marketing_rule_success(self, mock_openai_client, mock_openai_response, mock_prompt_data):
    ...
    @pytest.mark.asyncio
    async def test_marketing_rule_with_violation(self, mock_openai_client, mock_prompt_data):
...
```"""
    ...
    @pytest.mark.asyncio
    async def test_marketing_rule_formats_prompt_correctly(self, mock_openai_client, mock_openai_response, mock_prompt_data):
        """Test that PROMO rule formats the prompt and calls OpenAI correctly."""
        mock_openai_client.responses.create = AsyncMock(return_value=mock_openai_response)
        with patch('app.rules.base.client', mock_openai_client):
            ...
            assert input_messages[0]['role'] == 'system'
            content = input_messages[0]['content']
            assert "Listing description" in content
            assert "Marketing notes" in content
...
class TestRuleIntegration:
    ...
    @pytest.mark.asyncio
    async def test_all_rules_use_rate_limiter(self, mock_openai_client, mock_openai_response, mock_prompt_data):
        """Test that all rules integrate with rate limiter."""
        mock_openai_client.responses.create = AsyncMock(return_value=mock_openai_response)
        with patch('app.rules.base.client', mock_openai_client):
            # All rules should update rate limiter from response headers
            await get_fair_housing_violation_response("test", "test", prompt_data=mock_prompt_data)
            await get_comp_violation_response("test", "test", prompt_data=mock_prompt_data)
            await get_marketing_rule_violation_response("test", "test", prompt_data=mock_prompt_data)

            # Should have made 3 API calls
            assert mock_openai_client.responses.create.call_count == 3

    @pytest.mark.asyncio
    async def test_all_rules_use_retry_handler(self, mock_openai_client, mock_openai_response, mock_prompt_data):
        ...
        with patch('app.rules.base.client', mock_openai_client):
            with patch('asyncio.sleep', new=AsyncMock()):
                ...
                result1 = await get_fair_housing_violation_response("test", "test", prompt_data=mock_prompt_data)
                result2 = await get_comp_violation_response("test", "test", prompt_data=mock_prompt_data)
                result3 = await get_marketing_rule_violation_response("test", "test", prompt_data=mock_prompt_data)
